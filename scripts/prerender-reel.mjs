...
function injectRouteHtml(indexHtml, renderedApp, metadata) {
  ...
  const image = `${siteOrigin}${assetRoot}/reel-icon-128.png`;
  const headTags = [
    `<link rel="canonical" href="${escapeAttribute(canonical)}" />`,
    `<link rel="icon" type="image/png" sizes="128x128" href="${assetRoot}/reel-icon-128.png" />`,
    `<link rel="apple-touch-icon" href="${assetRoot}/reel-icon-128.png" />`,
    ...
    `<script id="reel-json-ld" type="application/ld+json">${escapeScript(JSON.stringify(metadata.schema))}</script>`,
  ].join('\n    ');
...
try {
  ...
  const { ReelProductSite, getReelMetadataForPath } = await vite.ssrLoadModule(
    '/src/products/reel/index.ts',
  );

  for (const routePath of routePaths) {
    const metadata = getReelMetadataForPath(routePath);
    const renderedApp = renderToString(React.createElement(ReelProductSite, { routePath }));
