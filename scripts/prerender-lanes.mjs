...
function injectRouteHtml(indexHtml, renderedApp, metadata) {
  ...
  const image = `${siteOrigin}${assetRoot}/lanes-icon.png`;
  const headTags = [
    `<link rel="canonical" href="${escapeAttribute(canonical)}" />`,
    `<link rel="icon" type="image/png" sizes="32x32" href="${assetRoot}/favicon.png" />`,
    `<link rel="icon" type="image/png" sizes="128x128" href="${assetRoot}/favicon-128.png" />`,
    `<link rel="apple-touch-icon" href="${assetRoot}/lanes-icon-256.png" />`,
    ...
    `<script id="lanes-json-ld" type="application/ld+json">${escapeScript(JSON.stringify(metadata.schema))}</script>`,
  ].join('\n    ');
...
try {
  ...
  const { LanesProductSite, getLanesMetadataForPath } = await vite.ssrLoadModule(
    '/src/products/lanes/index.ts',
  );

  for (const routePath of routePaths) {
    const metadata = getLanesMetadataForPath(routePath);
    const renderedApp = renderToString(React.createElement(LanesProductSite, { routePath }));
